...
public extension UITextView {
    ...
    func drawListMarkers(_ markers: [ListMarkerInfo]) {
        ...
        if let existing = hostLayer.sublayers?.first(where: { $0.name == listMarkerContainerLayerName }) {
            container = existing
            // Remove ALL previous marker sublayers.
            container.sublayers?.forEach { $0.removeFromSuperlayer() }
        } else {
            container = CALayer()
            container.name = listMarkerContainerLayerName
        ...
        // `displayScale` is 0 while the view is not attached to a screen (e.g. in unit tests).
        let contentsScale = max(traitCollection.displayScale, 1)
...
/// Name of the layer hosting the list marker layers, so it can be found again on the next update.
private let listMarkerContainerLayerName = "ListMarkerContainer"
...
private final nonisolated class ListMarkerLayer: CATextLayer {
    override init() {
        super.init()
    }

    init(string: NSAttributedString, frame: CGRect, contentsScale: CGFloat) {
        super.init()
        self.frame = frame
        self.string = string
        isWrapped = false
